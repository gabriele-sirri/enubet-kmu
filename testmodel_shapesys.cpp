...
void setTrueFrac(int n_par, double sigma, RooRealVar* f[],
		TMatrixDSym& cov_f);
...
int testmodel(RooWorkspace& w)
{
	...
	double N_TL_pot =  7.97468e+07;// total POT in TL production (current number from TL_r5_v5_clean)
	double N_TL_K = 16650;// total K in TL production (current number from TL_r5_v5_clean)
	double N_prod_K = 16650;// total K in production for systematics study (1x production from TL_r5_v5_clean)
	double N_TL_evt = 4095;// total events in TL production (integral of distributions for neutrino and background)

  double N_data = N_TL_evt*(N_data_pot/N_TL_pot)*(N_TL_K/N_prod_K);// normalization factor to scale variables to actual data-taking POT

	// generate toy data
	cout << "\nWill generate a total of " << N_data << " events, corresponding to " << N_data_pot << " POT\n" << endl;

	RooDataHist* data = model->generateBinned(RooArgSet{*evis, *z}, N_data, RooFit::Verbose(1));
	...
	model->plotOn(z_frame_pref, RooFit::Components("bkg_sys"), RooFit::FillColor(1),
	...
	// fit model to data
	auto fitresult = model->fitTo(*data, RooFit::ExternalConstraints(c_Vf), RooFit::Constrain(*alpha),
		...
		TMatrixDSym& cov_f)
{
	...
	// - U is the transformation matrx to apply to the the uncorrelated x variables;
	// - V is the diagonal matrix containing the standard deviatons of the uncorrelated
	//   variables x;
	//
	...
	*U = svd->GetU();
	...
	// get the S matrix: in this case is equal to the U matrix, since M is simmetric
	...
	*S = svd->GetSig();
	...
	for (int p = 0; p < n_par; p++) w[p] = rnd->Gaus(0, TMath::Sqrt((double)(*S)[p]));
